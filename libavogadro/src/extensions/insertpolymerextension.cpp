...
namespace Avogadro {
  ...
  void InsertPolymerExtension::build()
  {
    ...
    qDebug() << monomerASMI << monomerBSMI << monomerCSMI;
    ...
    while (true) {
      ...
      currentSMI = fullSMI; // clean this to check the progress
      if (currentSMI.endsWith('/'))
        currentSMI.remove(currentSMI.length());
      if (currentSMI.endsWith('\\'))
        currentSMI.remove(currentSMI.length());
      if (currentSMI.endsWith('='))
        currentSMI.remove(currentSMI.length());
    ...
    if (m_FF->Setup(polymer)) {
      m_FF->SteepestDescent(500, 1.0e-4);
