...
namespace Avogadro
{
                      ...
                      const char *delimstr)
  {
	...
	if ( endpos == std::string::npos ) endpos=s.size()-1;
	...
	for ( unsigned int j=0; j < shells.at(i).size(); j++ )
	  {
	    ...
	    for ( unsigned int k=prev; k < gtoIndicies.at(i).at(j); k++ )
	      std::cout << "       e = " << gtoExponents.at(k) << " c = " << gtoCoefficients.at(k) << std::endl;
	  }
      }
  ...
  orbital GUKBasisSet::shellTypeFromString(std::string label)
  {
    ...
    */
    ...
    if (label.size() == 2 )
      {
	if ( label.compare(0, 2, "sp") == 0 ) return SP;
      }
	...
	if ( line.compare(0,10," =========") == 0 )
	  {
	    ...
	    gukBasis.gtoIndicies.at(  gukBasis.shellLabels.size()-1 ).push_back( gukBasis.gtoExponents.size() );
	    break;
	  }
  ...
  {
    /* 
       Loop through a series of columns of printed MO vectors & return the number of orbitals read in
    */
	...
	for ( unsigned int j=0; j < gukBasis.shells.at(i).size(); j++ )
	  {
	    ...
	    for ( unsigned int k=prev; k < gukBasis.gtoIndicies.at(i).at(j); k++ )
	      basis->addGTO( s, gukBasis.gtoCoefficients.at(k), gukBasis.gtoExponents.at(k) );
	  }
      }
    return;
    ...
    for ( unsigned int i=0; i < nBasis ; i++ )
	  ...
	  for ( unsigned int j=MOs.size(); j<moEnd; j++)
	    ...
	    //std::cout << "Adding actual vector " << i << std::endl;
   	  MOs.insert( MOs.end(), gukBasis.moVectors.at(i).begin(),gukBasis.moVectors.at(i).end() );
   	  }
      }
...
int main() {
  ...
  std::string dir="/home/jmht/Documents/AVOGADRO/gukparser/outputs/";
  std::string filename=dir+"jens.gukout";
  //std::string filename=dir+"/g03vsguk/guk.gukout";
