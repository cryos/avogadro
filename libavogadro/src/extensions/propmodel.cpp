...
namespace Avogadro {
  ...
  int PropertiesModel::columnCount(const QModelIndex &parent) const
  {
    ...
    switch (m_type) {
    case AtomType:
      return 5 + (m_molecule->numConformers()*3); // element, type, valence, formal charge, partial charge, [ x, y, z ] * numConformers
    case BondType:
      return 6 + m_molecule->numConformers() - 1;
    case AngleType:
      return 5 + m_molecule->numConformers() - 1;
    case TorsionType:
      return 6 + m_molecule->numConformers() - 1;
    /*case CartesianType:
      return 3;*/
  ...
  QVariant PropertiesModel::data(const QModelIndex &index, int role) const
  {
    ...
    if (m_type == AtomType) {
      if (static_cast<unsigned int>(index.row()) >= m_molecule->numAtoms() ||
          static_cast<unsigned int>(index.column()) > 5 + (3*m_molecule->numConformers()) )
        return QVariant();
  ...
  void PropertiesModel::updateCache() const
  {
    ...
    // Create a copy of the main molecule to get the data from.
    // We create a copy as we need to loop through the conformers
    // to generate the data, and this would alter the molecule
    Molecule *copy_molecule = new Molecule();
    *copy_molecule = *m_molecule;
    ...
    for ( unsigned int numConf=0; numConf < copy_molecule->numConformers(); numConf++ )
      {
    ...
    // Delete our copy of the molecule
