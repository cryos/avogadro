...
namespace Avogadro
{
  ...
  void CESuperCellBuilder::buildSuperCell()
  {
    ...
    case GT_SPHERE:
      ...
      // radius == distA, so fcc cell length...
      double cellLength = distA * 2.0 * sqrt(2.0);

      cellMatrix = m_ext->unconvertLength(m_ext->currentCellMatrix()).transpose();
      cellMatrix.col(0) = Eigen::Vector3d(cellLength);
      cellMatrix.col(1) = Eigen::Vector3d(cellLength);
      cellMatrix.col(2) = Eigen::Vector3d(cellLength);
      // This causes a crash
      //      m_ext->setCurrentCellMatrix(m_ext->convertLength(Eigen::Matrix3d(cellMatrix.transpose())));
