...
static Cell * get_bravais( SPGCONST Cell * cell,
...
Cell * ref_refine_cell( SPGCONST Cell * cell,
			const double symprec )
{
  return get_bravais( cell, symprec );
}
			       ...
			       const double symprec )
{
  ...
  mat_multiply_matrix_vector_id3( position,
				  sum_rot,
				  position );
  for ( i = 0; i < 3; i++ ) {
		       ...
		       SPGCONST double metric[3][3] )
{
  ...
  ahex = 2 * a * sin( angle / 2 );
  chex = a * sqrt( 3 * ( 1 + 2 * cos( angle ) ) ) ;
  lattice[0][0] = ahex / 2;
