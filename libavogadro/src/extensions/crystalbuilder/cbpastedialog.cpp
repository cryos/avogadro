...
namespace Avogadro {
  ...
  CBPasteDialog::CBPasteDialog(QWidget *parent,
                               const QString &text,
                               Molecule *mol)
    : QDialog(parent),
      m_molecule(mol),
  ...
  CBPasteDialog::CBPFormat
  CBPasteDialog::guessFormat(const QString &text)
  {
    if (m_format) {
      return m_format;
    }
  ...
  bool CBPasteDialog::isVaspFormat(const QString &text)
  {
  ...
  void CBPasteDialog::refreshInvalidFormat()
  {
  ...
  void CBPasteDialog::refreshVaspFormat()
  {
    ...
    QString idents = "";
    if (static_cast<unsigned int>(symbolList.size()) == m_numAtomTypes) {
        idents = symbolList.join(" ");
    }
    else {
  ...
  void alertBadVaspComposition(QWidget *parent)
  {
    QMessageBox::critical
      (parent,
       parent->tr("Bad Compostion"),
       parent->tr("The identities field must contain the same number of "
                  "space delimited entries as line 6 of the POSCAR."));
  }

  bool CBPasteDialog::acceptVaspFormat()
  {
