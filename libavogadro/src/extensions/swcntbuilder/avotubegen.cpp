                                     ...
                                     bool cap, bool dbonds)
{
  ...
  // Clean molecule object:
  this->cleanupMolecule();
...
void AvoTubeGen::cleanupMolecule()
{
  // HACK it looks like theres a bug somewhere in all of the setOBMol/OBMol
  // calls, so this function manually builds an Avogadro::Molecule using
  // standard API calls, and then resets m_molecule to this "clean" state.
  Avogadro::Molecule mcopy;

  // Copy atoms and bonds
  QList<Avogadro::Atom*> atoms = m_molecule->atoms();
  QList<Avogadro::Bond*> bonds = m_molecule->bonds();
  QHash<unsigned long, unsigned long> atomLUT; // key: old atom id, val: new
  for (QList<Avogadro::Atom*>::const_iterator it = atoms.constBegin(),
       it_end = atoms.constEnd(); it != it_end; ++it) {
    Avogadro::Atom *oldAtom = (*it);
    Avogadro::Atom *newAtom = mcopy.addAtom();
    atomLUT.insert(oldAtom->id(), newAtom->id());
    *newAtom = *oldAtom;
  }
  for (QList<Avogadro::Bond*>::const_iterator it = bonds.constBegin(),
       it_end = bonds.constEnd(); it != it_end; ++it) {
    Avogadro::Bond *oldBond = (*it);
    Avogadro::Bond *newBond = mcopy.addBond();
    *newBond = *oldBond;
    // Sync atom ids
    newBond->setBegin(mcopy.atomById(atomLUT.value(oldBond->beginAtomId())));
    newBond->setEnd(  mcopy.atomById(atomLUT.value(oldBond->endAtomId())));
  }

  // Reset m_molecule to mcopy's state.
  *m_molecule = mcopy;
